...
[![Version](https://img.shields.io/badge/version-0.2.4-blue.svg)](https://github.com/SysAdminDoc/AI-Usage_Tracker/releases)
...
1. Download `AI-Usage-Tracker-chrome-v0.2.4.zip` from the [Releases page](https://github.com/SysAdminDoc/AI-Usage_Tracker/releases/latest).
2. Unzip it anywhere.
...
1. Download `ai-usage-tracker-firefox-v0.2.4.xpi` from the [Releases page](https://github.com/SysAdminDoc/AI-Usage_Tracker/releases/latest).
...
```bash
unzip AI-Usage-Tracker-native-scheduler-v0.2.4.zip -d ai-usage-tracker-native-scheduler
...
| `api.anthropic.com`, `api.openai.com`, `api.github.com`, and `api.cursor.com` host access | Optional exact-origin permissions requested only when the matching provider credential is saved or enabled | Credentials are stored separately in session/memory mode by default, or in persistent local compatibility mode only when explicitly selected; they are never synced or included in settings/diagnostics exports. GitHub Copilot keeps its organization and username in local settings. |
...
| `openrouter.ai` host access | Optional exact-origin permission requested only when an OpenRouter key is configured | The key stays in the separate credential boundary and is omitted from sync, settings, and diagnostics exports. |
...
For packaged-browser lifecycle and accessibility coverage, run `npm run test:runtime`; it rebuilds Chrome and Firefox artifacts and exercises fresh isolated profiles across wide/narrow layouts, RTL/high contrast, reduced motion, axe checks, and dialog focus contracts under the invisible browser-testing contract.
...
**Does it save API keys?** The extension can save optional Anthropic and OpenAI organization admin keys, a GitHub token for Copilot seat activity, a Cursor team admin API key, a Google Cloud monitoring OAuth token for Gemini, or an OpenRouter key in a separate credential record. Session or memory-only storage is the default and is cleared when its browser/extension context ends; persistent local storage is an explicit compatibility option. Values are never displayed again and are omitted from sync, settings, and diagnostics exports. No mode is a password vault; revoke the credential at the provider when you no longer need it.
...
**How do webhook alerts work?** Settings → Notifications keeps webhook delivery off until you enable it and grant the configured endpoint origin. Rule events use one bounded request per delivery pass and at most three event-level passes with backoff; the default payload contains the event schema, rule ID, tone, and catch-up flag, while provider, bucket, percentage, reset, title, and body details require the separate opt-in. The endpoint URL and delivery status stay local and are not included in diagnostics.
